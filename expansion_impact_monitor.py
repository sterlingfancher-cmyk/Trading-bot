...
VERSION = "expansion-impact-monitor-2026-06-04-v1-manual"
...
def _positions(state: Dict[str, Any], core: Any = None) -> Dict[str, Any]:
    try:
        live_positions = getattr(core, "portfolio", {}).get("positions", {})
...
def _trades(state: Dict[str, Any], core: Any = None) -> List[Dict[str, Any]]:
    try:
        live_trades = getattr(core, "portfolio", {}).get("trades", [])
...
def _risk_controls(state: Dict[str, Any], core: Any = None) -> Dict[str, Any]:
    try:
        rc = getattr(core, "portfolio", {}).get("risk_controls", {})
...
def build_payload(core: Any = None) -> Dict[str, Any]:
    ...
    observed_outcomes = sum(1 for row in trades if _is_exit(row))
    ...
    payload = {
        ...
        "deltas": {
            "execution_rows_delta": execution_rows - BASELINE_EXECUTION_ROWS,
            "observed_outcomes_delta": observed_outcomes - BASELINE_OBSERVED_OUTCOMES,
    ...
    try:
        if core is not None and hasattr(core, "portfolio"):
            core.portfolio["expansion_impact_monitor"] = {
