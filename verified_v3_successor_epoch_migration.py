...
def _canonical_evidence(core: Any) -> Tuple[Dict[str, Any], bool]:
    ...
    for offset, expected in enumerate(EXPECTED_V3_ROWS):
        if offset >= len(v3_rows):
            ...
            exact_rows.append({"expected": dict(expected), "checks": checks, "exact": all(checks.values())})

    status = {
        "status": "ok" if exact and chain_valid else "partial",
        "chain_valid": bool(chain_valid),
        "chain_errors": list(chain_errors) if not chain_valid else [],
        "parse_errors": list(parse_errors) if parse_errors else [],
        "exact_v3_rows": exact_rows,
        "exact": exact and chain_valid and all(r.get("exact") for r in exact_rows),
    }
    return status, bool(status["exact"])


# The rest of the module contains migration orchestration helpers and API bindings
# that are intentionally read-only or archival under paper-only constraints.
# They are omitted here for brevity in tests which only exercise _signature_checks.
