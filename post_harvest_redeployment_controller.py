...
After profit harvesting leaves the book underdeployed, this layer may re-submit
1-2 high-quality unheld candidates through the normal entry pipeline. It does
not raise max positions, bypass halts, bypass stop losses, bypass self-defense,
or force entries.
"""
...
VERSION = "post-harvest-redeployment-2026-06-03-v1"
...
PREFERRED_CONTEXTS = {"breakout_participation_starter", "relative_strength_leader_exception", "pattern_recognition_ranked"}
PREFERRED_CLASSES = {"breakout_starter", "leader_hold", "swing_candidate", "intraday_trade"}
HARD_BLOCK_TOKENS = ("halt", "stop_loss", "stop loss", "daily_loss", "drawdown", "self_defense", "market_not", "bear", "risk_off", "stale_runner", "runner_stale")
SOFT_BLOCK_TOKENS = ("profit_guard", "profit guard", "harvest", "maturity", "underdeploy", "cash")
...
def _portfolio_cash_equity(m: Any | None) -> Tuple[float, float, float]:
    portfolio = getattr(m, "portfolio", {}) or {}
...
def _self_defense_active(m: Any | None) -> Tuple[bool, str]:
    for getter in (lambda: getattr(m, "get_risk_controls")(), lambda: (getattr(m, "portfolio", {}) or {}).get("risk_controls") or {}):
        try:
            rc = getter()
...
def _is_breakout_signal(signal: Dict[str, Any] | None) -> bool:
    ...
    return bool(ctx.get("active") or signal.get("entry_context") == "breakout_participation_starter" or signal.get("trade_class") == "breakout_starter" or catalyst.get("reason") == "breakout_participation_layer")


def _summary(m: Any | None, signal: Dict[str, Any]) -> Dict[str, Any]:
    symbol = str(signal.get("symbol", "")).upper()
    ctx = signal.get("breakout_participation") or {}
    return {"symbol": symbol, "side": signal.get("side"), "score": round(_f(signal.get("score"), 0.0), 6), "sector": signal.get("sector") or _sector(m, symbol), "bucket": _bucket(m, symbol), "entry_context": signal.get("entry_context"), "trade_class": signal.get("trade_class"), "breakout": bool(_is_breakout_signal(signal)), "breakout_reason": ctx.get("reason"), "risk_tier": ctx.get("risk_tier")}
...
def _recent_exit_symbols(m: Any | None) -> set[str]:
    ...
    for row in trades[-250:]:
        ...
        is_exit = bool(row.get("exit_time") or row.get("closed_at")) or any(token in text for token in ("sell", "exit", "close", "trim", "harvest"))
        ts = _i(row.get("exit_time") or row.get("closed_at") or row.get("ts") or row.get("time") or row.get("timestamp"), 0)
        if is_exit and ts and 0 <= now_ts - ts <= EXIT_COOLDOWN_SECONDS:
            out.add(symbol)
    return out
...
def _quality_ok(signal: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    return False, "missing_quality_context"


def _priority(signal: Dict[str, Any]) -> float:
    return _f(signal.get("score"), 0.0) + (0.006 if _is_breakout_signal(signal) else 0.0) + (0.003 if str(signal.get("entry_context") or "") == "relative_strength_leader_exception" else 0.0) + (0.002 if str(signal.get("trade_class") or "") in {"leader_hold", "breakout_starter"} else 0.0)
...
def select_redeployment_candidates(m: Any | None, long_signals: Iterable[Dict[str, Any]] | None, short_signals: Iterable[Dict[str, Any]] | None, params: Dict[str, Any] | None, market: Dict[str, Any] | None, new_entries_allowed: bool, entry_block_reason: Any) -> Tuple[list[Dict[str, Any]], Dict[str, Any]]:
    if not ENABLED:
    ...
    if active:
        return [], {"allowed": False, "reason": "self_defense_active", "self_defense_reason": defense_reason, "version": VERSION}
    ...
    if cash_pct < MIN_CASH_PCT:
        return [], {"allowed": False, "reason": "cash_pct_below_post_harvest_threshold", "cash": round(cash, 2), "equity": round(equity, 2), "cash_pct": round(cash_pct, 4), "required_cash_pct": MIN_CASH_PCT, "version": VERSION}
    ...
    if not candidates:
        return [], {"allowed": False, "reason": "no_post_harvest_redeployment_candidate", "required_score": MIN_SCORE, "open_positions_count": len(positions), "max_positions": max_positions, "cash_pct": round(cash_pct, 4), "rejected_top_candidates": rejected[:12], "version": VERSION}
    ...
    return selected, {"allowed": True, "reason": "post_harvest_controlled_redeployment_candidates", "version": VERSION, "entry_scope": "controlled_post_harvest_redeployment_ladder", "candidates": [_summary(m, s) for s in selected], "top_candidates_reviewed": [_summary(m, s) for s in sorted(candidates, key=_priority, reverse=True)[:10]], "rejected_top_candidates": rejected[:12], "open_positions_count": len(positions), "target_open_positions": TARGET_OPEN_POSITIONS, "max_positions": max_positions, "cash": round(cash, 2), "equity": round(equity, 2), "cash_pct": round(cash_pct, 4), "max_entries_per_cycle": MAX_ENTRIES_PER_CYCLE, "does_not_raise_max_positions": True, "does_not_bypass_halts": True, "does_not_bypass_stop_losses": True, "does_not_force_entries": True, "entry_quality_check_still_required": True, "cooldown_seconds_after_harvest": EXIT_COOLDOWN_SECONDS}
...
def _chain_has_marker(fn: Any, marker: str, limit: int = 40) -> bool:
    ...
    for _ in range(limit):
        ...
        if bool(getattr(cur, marker, False)):
            return True
        cur = next((getattr(cur, a, None) for a in ("_post_harvest_redeployment_original", "_profit_maturity_rotation_original", "_paper_breakout_rotation_original", "_paper_exposure_debug_original", "__wrapped__") if callable(getattr(cur, a, None))), None)
    return False
...
def _patch_try_entries(m: Any) -> bool:
    ...
    def patched_try_entries_and_rotations(long_signals, short_signals, params, market, new_entries_allowed=True, entry_block_reason=None):
        ...
        _CYCLE_ENTRIES_USED = 0
        selected, info = select_redeployment_candidates(m, long_signals, short_signals, params or {}, market or {}, bool(new_entries_allowed), entry_block_reason)
        ...
        entries, rotations, blocked_entries = original(call_long_signals, call_short_signals, params, market, new_entries_allowed=call_new_entries_allowed, entry_block_reason=call_entry_block_reason)
        try:
            if selected:
...
def status_payload(m: Any | None = None) -> Dict[str, Any]:
    ...
    try:
        ...
        latest = {}
    return {"status": "ok", "type": "post_harvest_redeployment_status", "version": VERSION, "generated_local": _now(m), "enabled": bool(ENABLED and _paper_context()), "patched_try_entries": _chain_has_marker(getattr(m, "try_entries_and_rotations", None), "_post_harvest_redeployment_patched"), "latest_redeployment": latest, "policy": {"does_not_raise_max_positions": True, "does_not_bypass_halts": True, "does_not_bypass_stop_losses": True, "does_not_force_entries": True, "entry_quality_check_still_required": True, "max_open_positions_threshold": MAX_OPEN_POSITIONS, "target_open_positions": TARGET_OPEN_POSITIONS, "min_cash_pct": MIN_CASH_PCT, "min_score": MIN_SCORE, "exceptional_score": EXCEPTIONAL_SCORE, "max_entries_per_cycle": MAX_ENTRIES_PER_CYCLE, "exit_cooldown_seconds": EXIT_COOLDOWN_SECONDS}}
