...
Python imports sitecustomize automatically at startup. This file attaches ML
shadow logging and review endpoints to app.py without replacing the large app.py.
It does not place trades or override risk controls.
"""
from __future__ import annotations
import hashlib, importlib.abc, importlib.machinery, json, math, os, sys, datetime as dt

VERSION = "ml-shadow-2026-05-07"
...
def entry_floor(state):
    return fnum(nested(state, ["feedback_loop", "dynamic_min_long_score"], None), fnum(nested(state, ["explain", "current_permission", "active_min_entry_score"], None), 0.0))
...
def feature_row(item, decision, state, appmod=None):
    ...
    row = {
        "logged_local": now_text(appmod), "date": today(appmod), "symbol": sym,
        "side": item.get("side") or item.get("direction") or "long",
        "bucket": item.get("bucket") or bucket(sym, appmod), "sector": item.get("sector") or sector(sym, appmod),
        ...
        "spy_5d_pct": fnum(market.get("spy_5d_pct"), 0.0), "qqq_5d_pct": fnum(market.get("qqq_5d_pct"), 0.0),
        "vix_5d_pct": fnum(market.get("vix_5d_pct"), 0.0), "rates_5d_pct": fnum(market.get("rates_5d_pct"), 0.0),
        "futures_action": futures.get("action") or "", "futures_bias": futures.get("bias") or futures.get("action") or "",
        "breadth_state": breadth.get("state") or "", "breadth_action": breadth.get("action") or "",
        "tech_leadership_active": bool(tech.get("active", False)), "tech_leadership_state": tech.get("state") or "",
        "precious_metals_state": precious.get("state") or "", "precious_metals_action": precious.get("action") or "",
        "entry_floor": round(entry_floor(state), 6), "cash": round(fnum(state.get("cash"), 0.0), 2), "equity": round(fnum(state.get("equity"), 0.0), 2),
        "realized_pnl_today": round(fnum(perf.get("realized_pnl_today"), 0.0), 2),
        "daily_loss_pct": round(fnum(risk.get("daily_loss_pct"), 0.0), 4), "intraday_drawdown_pct": round(fnum(risk.get("intraday_drawdown_pct"), 0.0), 4),
        "self_defense_active": bool(risk.get("self_defense_active") or feedback.get("self_defense_mode")),
        "future_outcome_pending": True, "future_return_close_pct": None, "future_return_next_day_pct": None,
        "source_hash": h(item),
    }
...
def ensure_ml(state):
    ...
    ml.setdefault("enabled", ENABLED)
    ml.setdefault("live_trade_decider", False)
...
def append_features(state, appmod=None):
    ...
    for item, decision in candidate_items(state):
        ...
        if row["row_id"] not in ids:
            ids.add(row["row_id"]); new_rows.append(row)
    if new_rows:
...
def prob(row, total):
    ...
    if row.get("tech_leadership_active") and row.get("sector") in {"XLK", "XLY"}: p += 0.035
    if row.get("bucket") == "precious_metals" and row.get("precious_metals_state") in {"safe_haven_bid", "trend_confirmed"}: p += 0.035
    if row.get("futures_action") in {"gap_chase_protection", "bearish_caution", "risk_off"}: p -= 0.025
    ...
    if confidence == "low_data": p = 0.50 + (p - 0.50) * 0.35
    elif confidence == "developing": p = 0.50 + (p - 0.50) * 0.65
    p = max(0.05, min(0.95, p))
    return {"symbol": row.get("symbol"), "side": row.get("side"), "bucket": row.get("bucket"), "sector": row.get("sector"), "rule_score": row.get("score"), "entry_floor": row.get("entry_floor"), "decision_seen": row.get("decision"), "ml_shadow_probability": round(p, 4), "confidence": confidence, "shadow_action": "rank_higher" if p >= 0.58 else "rank_lower" if p <= 0.45 else "neutral"}
...
def review(state, appmod=None):
    ...
    return {"version": VERSION, "enabled": ENABLED, "live_trade_decider": False, "rows_logged": total, "accepted_rows": sum(r.get("decision") == "accepted" for r in rows), "blocked_rows": sum(r.get("decision") == "blocked" for r in rows), "rejected_rows": sum(r.get("decision") == "rejected" for r in rows), "pending_outcome_rows": sum(bool(r.get("future_outcome_pending")) for r in rows), "bucket_summary": group(rows, "bucket"), "sector_summary": group(rows, "sector"), "latest_shadow_rankings": preds[:15], "readiness": {"ml_ready_for_live_decisions": False, "reason": "shadow data capture active; live ML requires enough rows plus walk-forward/backtest evidence", "recommended_next_threshold": "100+ logged scanner opportunities and 2-4 weeks of paper data before ML affects entries"}, "last_updated_local": now_text(appmod)}
...
def install_routes(appmod):
    ...
    if "/paper/ml-health" not in rules:
        ...
        def ml_health():
            state = load_state(appmod); ml = ensure_ml(state)
            return js({"status": "ok", "version": VERSION, "enabled": ENABLED, "live_trade_decider": False, "rows_logged": len(ml.get("feature_log", [])), "state_persistence_mode": getattr(appmod, "STATE_PERSISTENCE_MODE", None), "state_file": getattr(appmod, "STATE_FILE", None)})
    if "/paper/ml-dataset" not in rules:
        @flask_app.route("/paper/ml-dataset")
        def ml_dataset():
            state = load_state(appmod); ml = ensure_ml(state)
            try: limit = int(appmod.request.args.get("limit", "250"))
    ...
    if "/paper/ml-shadow" not in rules:
        ...
        def ml_shadow():
            ...
            return js({"status": "ok", "mode": "shadow_only", "live_trade_decider": False, "latest_shadow_rankings": r.get("latest_shadow_rankings", []), "readiness": r.get("readiness", {}), "plain_english": ["ML shadow mode logs and ranks scanner patterns only.", "It cannot place trades or override risk controls."]})
    if "/paper/ml-feature-log" not in rules:
        @flask_app.route("/paper/ml-feature-log")
        def ml_feature_log():
            state = load_state(appmod); ml = ensure_ml(state); rows = ml.get("feature_log", [])
    ...
    if "/paper/backtest-summary" not in rules:
        ...
        def backtest_summary():
            state = load_state(appmod); trades = state.get("trades", []); realized = state.get("realized_pnl", {}); risk = state.get("risk_controls", {})
            return js({"status": "ok", "version": VERSION, "type": "paper_replay_backtest_readiness", "note": "Full walk-forward simulation comes after enough ML feature rows are collected.", "rule_system_snapshot": {"trades_logged": len(trades), "realized_pnl_today": fnum(realized.get("today")), "realized_pnl_total": fnum(realized.get("total")), "wins_total": fint(realized.get("wins_total")), "losses_total": fint(realized.get("losses_total")), "intraday_drawdown_pct": fnum(risk.get("intraday_drawdown_pct")), "daily_loss_pct": fnum(risk.get("daily_loss_pct"))}, "ml_shadow_snapshot": review(state, appmod), "recommended_next_step": "Collect scanner rows during regular sessions, then compare rules vs ML ranking on profit factor, stop-loss rate, and missed-winner rate."})


def patch_save(appmod):
    global _PATCHING
    if not hasattr(appmod, "save_state") or getattr(appmod.save_state, "_ml_shadow_patched", False): return
    original = appmod.save_state
    def patched(state):
        global _PATCHING
        if _PATCHING: return original(state)
        try:
            _PATCHING = True; append_features(state, appmod)
        finally:
            _PATCHING = False
...
def install(appmod):
    ...
    patch_save(appmod); install_routes(appmod); _INSTALLED = True
...
class Loader(importlib.abc.Loader):
    ...
    def exec_module(self, module):
        ...
        if getattr(module, "__name__", "") == "app": install(module)
...
if ENABLED and not any(isinstance(x, Finder) for x in sys.meta_path): sys.meta_path.insert(0, Finder())
if "app" in sys.modules:
    try: install(sys.modules["app"])
