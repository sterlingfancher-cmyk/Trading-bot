...
VERSION = "surge-state-repair-2026-06-08-v1-spy-position-accounting"
...
def _compute_pnl(entry: float, last_price: float, shares: float) -> Dict[str, float]:
    ...
    return {"pnl_dollars": round(pnl_dollars, 4), "pnl_pct": round(pnl_pct, 4)}
...
def _status(core: Any = None) -> Dict[str, Any]:
    ...
    spy = positions.get("SPY") if isinstance(positions.get("SPY"), dict) else None

    broken = False
    if spy is not None:
        broken = _safe_float(spy.get("entry")) <= 0.0 or _safe_float(spy.get("shares")) <= 0.0
    ...
    return {
        ...
        "repair_plan": {
            ...
            "reason": "Cash was already deducted by the prior paper surge executor run; complete the malformed position only.",
        },
        "guardrails": {
            "does_not_trade": True,
...
def apply_repair(core: Any = None, confirm: bool = False) -> Dict[str, Any]:
    ...
    state = _load_state(core)
    positions = _positions(pf, state)
    spy = positions.get("SPY") if isinstance(positions.get("SPY"), dict) else {}
    ...
    # Recompute equity if cash exists. Do not change cash; the prior executor already deducted it.
    cash = _safe_float(pf.get("cash", state.get("cash", 0.0)))
    market_value = 0.0
    unrealized = 0.0
    for pos in positions.values():
        ...
        market_value += qty * lp
        unrealized += (lp - ent) * qty if qty and ent else _safe_float(pos.get("pnl_dollars"), 0.0)
